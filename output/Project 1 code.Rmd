...
In this project, we want to focus on the school. So, let's have a look at want kind of schools we have.
...
## The number of Aristotle' Work and Plato's Work
```{r}
aristotle_work<-data%>%
...
hist(aristotle_length,breaks=15,main = "Histogram of Plato's Sentence-Length")
...
sum(aristotle_length)/aristotle_work_no
```

```{r}
plato_work_no<-dim(plato_work)[1]
sum(plato_length)/plato_work_no
```
